...
#define ROM_NMI_SMOKE_PATH "tests/rom_nmi_smoke.bin"
#define ROM_NMI_EXPECTED_LATCH 0xB7
...
int main(void) {
  long rom_size = file_size(ROM_NMI_SMOKE_PATH);
  assert(rom_size > 0);
  ...
  assert(kaypro_sysport_latch(m->sysport) == ROM_NMI_EXPECTED_LATCH);
  assert(m->bank1);

  kaypro_destroy(m);
  printf("kaypro rom nmi smoke: ok\n");
  return 0;
}